				      ...
				      loff_t *ppos)
{
	...
	case DBGFS_BIST_TONE:
		...
		if (ret == 3)
			txNcoConfig.attenCtrl = ADI_ADRV904X_TX_TEST_NCO_ATTEN_0DB & val3;
		else
	...
	enum jesd204_state_op_reason reason)
{
	...
	struct adrv904x_jesd204_priv *priv = jesd204_dev_priv(jdev);
	static adi_adrv904x_AdcSampleXbarCfg_t adcXbar = { 0 };
	...
	ret = adi_adrv904x_AdcSampleXbarGet(phy->kororDevice, ADI_ADRV904X_FRAMER_0, &adcXbar);
	if (ret) {
	...
	ret = adi_adrv904x_AdcSampleXbarSet(phy->kororDevice, ADI_ADRV904X_FRAMER_0, &adcXbar);
	if (ret) {
